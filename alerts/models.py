...
COVERAGE_MEASUREMENT_MODELS = models.Q(
    app_label="eo_sensors", model="coveragemeasurement"
)
...
RULE_MODELS = (
    models.Q(model="scopetyperule")
    | models.Q(model="scoperule")
    | models.Q(model="parameterrule")
)
...
class ScopeTypeRule(models.Model):
    ...
    def serialize(self):
        return json.dumps(
            dict(
                scope_type=self.scope_type,
                measurement_content_type_str=str(self.measurement_content_type),
...
class ScopeRule(models.Model):
    ...
    def serialize(self):
        return json.dumps(
            dict(
                scope_id=self.scope and self.scope.pk,
...
class ParameterRule(models.Model):
    ...
    def serialize(self):
        return json.dumps(
            dict(
                station_id=self.station and self.station.id,
...
class Alert(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    rule_content_type = models.ForeignKey(
        ContentType,
        on_delete=models.CASCADE,
    ...
    measurement_content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, limit_choices_to=MEASUREMENT_MODELS
    )
    measurement_id = models.PositiveIntegerField()
    measurement = GenericForeignKey("measurement_content_type", "measurement_id")
