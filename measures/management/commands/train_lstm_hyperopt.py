...
class Command(BaseCommand):
    ...
    def train_lstm(self,script_config,since_date):
        ...
        date_since = datetime(int(since_date[0]),int(since_date[1]),int(since_date[2]))
        raw_dataset = self.read_time_series_from_db(target_sensor,date_col,hr_col,'minute',numeric_var,sensor_var,date_since)
        ...
        self.log_success("Got time series dataset of shape {} with columns {}".format(time_series_dset.shape,time_series_dset.columns))

        sup_dataset,scaler = get_dataset_from_series(time_series_dset,n_past_steps)
        self.log_success("Got supervised dataset of shape {} with columns {}".format(sup_dataset.shape,sup_dataset.columns))
        ...
        for key in dataset_splits.keys():
            self.log_success("{} shapes: {},{}".format(key,dataset_splits[key]['X'].shape,dataset_splits[key]['y'].shape))
        ...
        self.log_success("LSTM NNNet hyperpars to optimize on: mults:{}, dropout:{}, n mid layers:{}".format(
        ...
        space = hp.choice('nnet_config',[
            {
                ...
                'dropout_rate_1' : hp.uniform('dropout_rate_1',dropout_rate_range[0],dropout_rate_range[1]),
                'mult_mid': hp.choice('mult_mid',mults),
                'dropout_rate_mid' : hp.uniform('dropout_rate_mid',dropout_rate_range[0],dropout_rate_range[1]),
                'mult_n': hp.choice('mult_n',mults),
                'dropout_rate_n' : hp.uniform('dropout_rate_n', dropout_rate_range[0],dropout_rate_range[1]),
        ...
        self.log_success("Hyper parameter optimization for optimal pars {} took {} seconds for {} datapoints".format(
            optimal_pars,
            opt_time,
            trainset['X'].shape[0]
            )
        ...
        self.log_success("Build LSTM NNet with optimal parameters \n {}".format(lstm_nnet_arq.summary()))
        ...
        self.log_success("Trained LSTM NNet {} took {} seconds for {} datapoints".format(
            lstm_nnet,
            train_time,
            trainset['X'].shape[0])
            )
        ...
        self.log_success("Train MAE {} and took {} seconds".format(train_mae,train_eval_time))
        ...
        self.log_success("Test MAE {} and took {} seconds".format(test_mae,test_eval_time))
        ...
        results.to_csv('{}{}_hyperopt_results_{}.csv'.format(output_results_path,str(script_config),time_stmp_str),index=False)
        ...
        with open('{}{}_model_hyperopt_package_{}.model'.format(output_models_path,str(script_config),time_stmp_str), 'wb') as file_pi:
    ...
    def add_arguments(self, parser):
        parser.add_argument('since_date', type=str, help='YYYY-MM-DD date since when we are fetching measures to train')
