...
class Command(BaseCommand):
    def process_measure_row(self, station, measure):
        ...
        for minute in [0, 15, 30, 45]:
            ...
            timestamp = datetime(the_year, the_mo, the_day, the_hr, minute, 0,
                                 0)
            ...
            try:
                new_measure, created = Measure.objects.get_or_create(
                    datetime=timestamp, station=station, attributes=attributes)

                if created:
                    self.log_success('Measure created: {}'.format(new_measure))
                else:
                    self.log_success(
                        'Measure already exists'.format(new_measure))
            except RuntimeError as e:
                self.log_error('Exception trying to create measure'.format(e))
    ...
    def handle(self, *args, **options):
        ...
        for city in unique_cities:
            ...
            for station_code in unique_stations:
                ...
                dataset.loc[dataset.inme == station_code].apply(
                    lambda x: self.process_measure_row(station, x), axis=1)
